...
const row = (id: string, t: number, hour: number | null, opts: { coverImage?: string; source?: string; dups?: Row[] } = {}): Row =>
    ({ id, t, hour, ...opts });
const day = (key: string, events: Row[]): Day =>
    ({ key, label: key, short: key, events });
...
describe('filterDaysBySource', () => {
    ...
    it('behåller bara valda källors rader', () => {
...
describe('mergeListedDays', () => {
    const base: Day[] = [
        day('2026-09-03', [row('a', 300, 10, { coverImage: 'img' }), row('b', 100, 8)]),
    ...
    it('samma dag: raderna slås ihop, bildsatta först, tidsordning inom grupperna', () => {
        const extra = [day('2026-09-03', [row('x', 200, 9, { coverImage: 'img' }), row('y', 50, 7)])];
