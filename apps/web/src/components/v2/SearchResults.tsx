...
export interface CityHit {
    ...
    /** "jazz göteborg": eventdelen av söktexten — siffran räknar bara den. */
    matchText?: string;
}
...
interface SearchResultsProps {
    ...
    /** Sökfältet är utfällt — då visas panelen även utan söktext (bara
     *  kategoriraden, 16/9). */
    open?: boolean;
    /** Kategoriraden (CategoryChipRow) — överst i panelen när sökningen är öppen. */
    chips?: React.ReactNode;
    /** Ort som tolkats ur söktexten ("jazz göteborg") — rubriken säger var. */
    cityName?: string;
    /** Söktexten UTAN ortnamnet — fetstilen markerar bara den delen. */
    highlightQuery?: string;
}

/**
 * Resultatpanel under sökfältet. Tre delar:
 *
 * 0. KATEGORIRADEN överst (16/9) — syns så fort sökfältet är öppet, även
 *    innan man skrivit något. Kategorifiltret bor här sedan kolumnen till
 *    höger revs 15/9.
 * 1. STÄDER — klick flyger kartan dit. Tillkom 10/8 efter en
 *    användarkommentar: det fanns bara eventsök, så vägen till sin egen ort
 *    gick via att skrolla Sverigekartan och zooma in för hand. Söker man
 *    "jazz göteborg" är den tolkade orten den enda raden.
 * 2. EVENT — sökningen täcker alla kommande dagar (inte bara den valda),
 *    och klick hoppar till eventets dag, väljer det och flyger dit.
 */
export default function SearchResults({
    query, results, onPick, cities = [], onPickCity, open = false, chips, cityName, highlightQuery,
}: SearchResultsProps) {
    const [visibleCount, setVisibleCount] = useState(PAGE_SIZE);
    useEffect(() => { setVisibleCount(PAGE_SIZE); }, [query]);

    const q = normalizeSearchQuery(query);
    const showChips = open && !!chips;
    if (!q && !showChips) return null;

    const hasCities = cities.length > 0 && !!onPickCity;
    const where = cityName ? ` i ${cityName}` : '';

    return (
        // z-[1165]: samma lager som Sparat-/profilpanelen — ÖVER navbaren (1160)
        // och stadsrutans dag/vecka-väljare (1090). Låg tidigare på 1040 och
        // hamnade då UNDER både knappkolumnerna och dagväljaren (Josef 31/8).
        ...
        <div className="absolute top-[4.6rem] right-4 left-4 sm:left-auto sm:w-[420px] z-[1165] pointer-events-auto">
            <div className="dark rounded-2xl bg-slate-900/80 backdrop-blur-md border border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[min(60vh,30rem)] animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                {showChips && (
                    <div className={`shrink-0 ${q ? 'border-b border-white/10' : ''}`}>
                        {chips}
                    </div>
                )}
                ...
                {q && (
                    <>
                        {/* Städer — egen sektion, alltid synlig (scrollar inte
                        ...
                        {hasCities && (
                            <div className="shrink-0 border-b border-white/10">
                                ...
                                <ul className="divide-y divide-white/10">
                                    {cities.map(({ city, weekCount, matchText }) => (
                                        <li key={city.name}>
                                            <button
                                                type="button"
                                                ...
                                                <span className="flex-1 min-w-0">
                                                    ...
                                                    <span className="block text-xs text-white/55 truncate">
                                                        {weekCount === null
                                                            ...
                                                            : matchText
                                                                ? (weekCount === 0
                                                                    ? `Inget med ”${matchText}” den närmaste veckan`
                                                                    : `${weekCount} med ”${matchText}” den närmaste veckan`)
                                                                : weekCount === 0
                        ...
                        <div className="px-4 py-2.5 border-b border-white/10 shrink-0">
                            <span className="text-[10px] font-black uppercase tracking-widest text-white/45">
                                {results.length === 0
                                    ? `Inga event matchar${where}`
                                    : `${results.length} ${results.length === 1 ? 'träff' : 'träffar'}${where} · alla dagar`}
                            </span>
                        </div>
                        {results.length === 0 ? (
                            hasCities ? (
                                ...
                                <div className="px-6 py-8 flex flex-col items-center gap-2 text-center">
                                    ...
                                    <p className="text-xs text-white/45">Sök på stad, titel, plats, arrangör eller kategori — eller båda, t.ex. ”jazz göteborg”.</p>
                                </div>
                            )
                        ) : (
                            <ul className="overflow-y-auto divide-y divide-white/10 custom-scrollbar">
                                {results.slice(0, visibleCount).map(evt => (
                                    <EventListRow key={evt.id} evt={evt} onPick={onPick} highlight={highlightQuery ?? q} />
                                ))}
