...
export default function SavedPanel({ open, events, savedEventIds, onPick, onRemove, onClose }: SavedPanelProps) {
    ...
    return (
        <>
            {/* Klick utanför panelen stänger den. z-[1164]: direkt under
                panelen, så ett klick på det krom panelen numera täcker
                (stadsrutan, kategorikolumnen, navbarens knappar) stänger den
                i stället för att gå igenom till knappen under. */}
            <div className="fixed inset-0 z-[1164]" onClick={onClose} />
            {/* z-[1165]: ÖVER navbaren (1160), stadsrutan (1090) och
                kategorikolumnen (1150) — panelen låg först på 1155 "under
                navbaren så hjärtknappen är kvar överst", men på mobilen
                målades då navbarens vänsterkolumn (profil/hjärta/plus) OVANPÅ
                panelinnehållet så man inte såg allt (Josef 21/8). Fortfarande
                under eventkortet (1250)/modalerna (1300). */}
