...
export default function LinkEventCard({ linkEvent, isAdmin = false, distance, onDelete, isPanelMode = false, showFullAddress = false, onRevealStepChange, initialRevealStep = 0, alwaysExpanded = false, onContentTap, saved = false, onToggleSave, canDelete = false, onDeleteOwn, canEdit = false, onEditOwn, onBoost, activityView = false, onToggleActivityView, nearbyView = false, onToggleNearbyView, onBackToGroup, backToGroupCount = 0, groupIndex = 0, groupTotal = 1, onGroupNext, hasStar = false, canPlaceStar = false, onPlaceStar }: LinkEventCardProps) {
    ...
    // Helskärmsvisning av omslagsbilden — för ALLA event (ägarbeslut 14/9
    // kväll: "man ska kunna se hela bilden på alla, oavsett ifall de är
    // scrapade eller så"; började som VADKUL-värdade-only samma morgon, tips
    // på eftermiddagen). Affischer bär ofta all info I bilden och h-56-
    // beskärningen gjorde dem oläsliga. Bildklicket som stegade reveal på
    // skrapade event är därmed ERSATT — beskrivningsytan stegar som förut.
            ...
            <div className="flex flex-col w-full">
                    ...
                    {hasRealCover && !coverFailed && (
                        ...
                        >
                            ...
                            />
                            {/* Klick → HELSKÄRM, INTE 21/8-expansionen i kortet
                                (riven: liggande bilder blev LÄGRE än h-56-
                                beskärningen och klicket såg trasigt ut — helskärm
                                funkar åt båda hållen). Chipen berättar att bilden
                                går att trycka. */}
