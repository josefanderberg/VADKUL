...
exports.scrapeTickster = scrapeTickster;
...
function guessCategoryFromTitle(title) {
    ...
    if (t.includes('musik') || t.includes('konsert') || t.includes('kör') || t.includes('orkester') || t.includes('tour'))
        return 'music';
    if (t.includes('sm i') || t.includes('cup') || t.includes('lopp') || t.includes('sport') || t.includes('match') || t.includes('tävling') || t.includes('lakers') || t.includes('hockey'))
        return 'sport';
    if (t.includes('quiz') || t.includes('spel') || t.includes('boardgame') || t.includes('bingo'))
        return 'game';
    if (t.includes('teater') || t.includes('musikal') || t.includes('standup') || t.includes('humor') || t.includes('konst') || t.includes('utställning') || t.includes('komedi') || t.includes('magic'))
        return 'culture';
    if (t.includes('mat') || t.includes('öl') || t.includes('vin') || t.includes('dinner') || t.includes('tasting') || t.includes('provning'))
        return 'food';
    if (t.includes('marknad') || t.includes('loppis') || t.includes('mässa') || t.includes('megaloppis'))
        return 'market';
    ...
    if (t.includes('träning') || t.includes('yoga') || t.includes('gym') || t.includes('fitness'))
        return 'training';
    if (t.includes('öppet hus') || t.includes('föreläsning') || t.includes('workshop') || t.includes('seminarium') || t.includes('mässa'))
        return 'study';
    if (t.includes('campus') || t.includes('student') || t.includes('kår'))
        return 'campus';
    return 'other';
}
async function scrapeTickster() {
    ...
    for (const evt of uniqueEvents) {
        try {
            ...
            const exists = await (0, dbHelper_1.eventExistsInDb)(evt.href);
            if (exists) {
                ...
                try {
                    ...
                    if (data['@type'] === 'Event') {
                        ...
                        if (data.location && data.location.name)
                            jsonLdLocation = data.location.name;
                        ...
                        if (data.startDate) {
                            ...
                            if (jd.includes('T')) {
                                const timePart = (_a = jd.split('T')[1]) === null || _a === void 0 ? void 0 : _a.substring(0, 5);
            ...
            if (jsonLdLat && jsonLdLng) {
                ...
                const coords = await (0, venueCoordinates_1.geocodeVenue)(finalLocation);
            ...
            await (0, dbHelper_1.addEventToDb)(linkEvent);
...
//# sourceMappingURL=tickster.js.map